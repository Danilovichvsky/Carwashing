...
class AnonymousBookingForm(forms.ModelForm):
    ...
    def clean_phone(self):
        ...
        if phone:
            # Регулярное выражение для формата телефона (код Украины +380 и 9 цифр)
            phone_pattern = r'^\+380\d{9}$'

            # Проверка формата телефона
            if not re.match(phone_pattern, phone):
                raise ValidationError("Телефон должен быть в формате +380935042254.")
            ...
            operator_code = phone[3:6]  # Извлекаем код оператора (с учетом префикса +380)
